...
WS_PREFIX=${WS_PREFIX:-http://127.0.0.1:8080/}
...
SERVER_ARGS=(run --project "$SERVER_PROJ" -- --ws-prefix "$WS_PREFIX")
WS_URL="$WS_PREFIX"
WS_URL="${WS_URL/http:\/\//ws://}"
WS_URL="${WS_URL/https:\/\//wss://}"
CLIENT_ARGS=(run --project "$CLIENT_PROJ" -- --ws "$WS_URL" --username "$USERNAME")
